...
words.forEach(function(value, index) {
	...
	paragraph = paragraph.map(function(val) {
		return val.replace(re, function(match) {
			return "<sp data-html = 'true' title = '" + "<h4>" + value[0].substring(0, 1) + "<b></b>" + value[0].substring(1, value[0].length) + "</h4>" + words[index][1] + "' class = 'vocab'><x>" + match.substring(0, 1) + "<b></b>" + match.substring(1, match.length) + "</x></sp>"
		});
	});
});
